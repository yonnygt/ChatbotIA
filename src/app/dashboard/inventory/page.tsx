        ...
        <div className="flex flex-col min-h-dvh bg-background-dark text-white">
            ...
            <header className="px-5 pt-12 pb-4">
                <div className="flex items-center justify-between">
                    <div>
                        <p className="text-xs font-medium text-primary/70 uppercase tracking-wider">Gestión</p>
                        <h1 className="text-2xl font-extrabold text-white tracking-tight mt-1">Inventario</h1>
                    </div>
                    <div className="flex items-center gap-2">
                        {outOfStockCount > 0 && (
                            <div className="flex items-center gap-1.5 rounded-full bg-red-500/10 border border-red-500/15 px-3 py-1.5">
                                <span className="material-symbols-outlined text-red-400 text-[14px]">warning</span>
                                <span className="text-xs font-bold text-red-400">{outOfStockCount} agotados</span>
                            </div>
                        )}
                        <button
                            onClick={() => setModalProduct(null)}
                            className="h-10 w-10 rounded-xl bg-primary flex items-center justify-center shadow-glow hover:brightness-110 active:scale-95 transition-all"
                        >
                            <span className="material-symbols-outlined text-background-dark text-[22px]">add</span>
                        </button>
                    </div>
                </div>
            </header>
            ...
            <div className="px-5 mb-3">
                <div className="flex items-center gap-3 rounded-2xl bg-surface-dark border border-white/5 px-4 py-3">
            ...
            <div className="flex gap-2 px-5 mb-4 overflow-x-auto scrollbar-hide">
                <button
                    ...
                    className={`shrink-0 rounded-full px-3.5 py-2 text-xs font-bold transition-all duration-200 ${selectedCategory === "all"
                        ...
                        : "bg-surface-dark/60 text-gray-400 border border-white/5 hover:border-primary/20"
                        }`}
                >
                    Todos
                </button>
            ...
            <main className="flex-1 px-5 pb-28 space-y-2.5">
                {loading ? (
                    <div className="space-y-2.5">
                        {[1, 2, 3, 4].map((i) => (
                            <div key={i} className="h-20 rounded-2xl bg-surface-dark/50 animate-pulse" />
                        ))}
                    </div>
